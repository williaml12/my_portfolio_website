    ...
    with st.container():
        ...
        for chat in st.session_state.conversation:
            with st.container():
                ...
                with col2:
                    st.markdown(f'<div class="user-message"><strong>User:</strong> {}</div>', unsafe_allow_html=True)
            with st.container():
                col1, col2 = st.columns([1, 22])
                with col1:
                    st.image(bot_icon_url, width=30)
                with col2:
                    st.markdown(f'<div class="bot-message"><strong>AI Bot:</strong> {}</div>', unsafe_allow_html=True)
